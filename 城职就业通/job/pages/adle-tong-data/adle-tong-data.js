  ...
  i_getData() {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        chart.on('finished', () => {
          that.selectComponent('#mychart-dom-i_bar').canvasToTempFilePath({
            success: function (res) {
              that.setData({
                i_echartImgSrc: res.tempFilePath
              })
            },
            fail: err => console.log('转换图片失败', err)
          });
        })
        wx.hideLoading();
      }
  ...
  a_getData() {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        chart.on('finished', () => {
          that.selectComponent('#mychart-dom-a_bar').canvasToTempFilePath({
            success:function(res){
              console.log("转换图片成功")
              that.setData({
                a_echartImgSrc: res.tempFilePath
              })
            },
            fail: err => console.log('转换图片失败', err)
          });
