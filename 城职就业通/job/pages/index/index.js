  ...
  stu_login:function(e){
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data[0].stu_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 0) {
        ...
        }else if(res.data[0].stu_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 1) {
          wx.showToast({
            title: '账号不存在',
            icon: 'loading',
            duration: 1500,
  ...
  ter_login:function(e){
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        console.log("输入账号为：" + e.detail.value.accountNumber + "输入密码为：" + e.detail.value.password);
        if (res.data[0].teach_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 0) {
          //如果输入结果符合查询结果，将要执行的代码
          wx.showLoading({
            title: '登录中！',
            mask: true,
        ...
        }else if(res.data[0].teach_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 1) {
          wx.showToast({
            title: '账号不存在',
            icon: 'loading',
            duration: 1500,
  ...
  leader_login: function (e) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        console.log("输入账号为：" + e.detail.value.accountNumber + "输入密码为：" + e.detail.value.password);
        if (res.data[0].lead_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 0) {
          //如果输入结果符合查询结果，将要执行的代码
          wx.showLoading({
            title: '登录中！',
            mask: true,
        ...
        }else if(res.data[0].lead_code == e.detail.value.accountNumber && res.data[0].password == e.detail.value.password && res.data[0].state == 1){
          wx.showToast({
            title: '账号不存在',
            icon: 'loading',
            duration: 1500,
